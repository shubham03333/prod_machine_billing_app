...
export async function PUT(
    ...
    if (isNaN(additionalNum) || additionalNum < 0) {
      return NextResponse.json(
        { error: "Invalid additional amount" },
        { status: 400 },
      );
    }
    ...
    if (
      normalHourlyRate &&
      normalHourlyRate !== "" &&
      (isNaN(normalHourlyRateNum) || normalHourlyRateNum < 0)
    ) {
      return NextResponse.json(
        { error: "Invalid normal hourly rate" },
        { status: 400 },
      );
    }
    ...
    if (
      breakerHourlyRate &&
      breakerHourlyRate !== "" &&
      (isNaN(breakerHourlyRateNum) || breakerHourlyRateNum < 0)
    ) {
      return NextResponse.json(
        { error: "Invalid breaker hourly rate" },
        { status: 400 },
      );
    }
